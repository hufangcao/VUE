        ...
        <form @submit.prevent="login">
          ...
          <div :class="{on:!loginWay}">
            <section>
              ...
              <section class="login_verification">
                <input type="text"  placeholder="密码" v-model="pwd" v-show="showPwd" >
                <input type="password" placeholder="密码" v-model="pwd"  v-show="!showPwd" >
                {{showPwd}}
    ...
    methods:{
      ...
      async login(){
        if(this.loginWay){
          ...
          if(!this.rightPhone){
            console.log('您输入的电话号码有误错误')
          }else if(!/^\d{6}$/.test(code)) {
            console.log('您的验证码输入错误')
          }
          if(rightPhone&&/^\d{6}$/.test(code)){
